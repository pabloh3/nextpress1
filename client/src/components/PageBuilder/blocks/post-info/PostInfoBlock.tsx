...
import { Info, Calendar, Folder, Tag, Clock, Settings, Wrench } from "lucide-react";
...
const DEFAULT_CONTENT: PostInfoContent = {
  postId: "", showDate: true, showCategories: true,
  showTags: true, showReadTime: true,
  dateFormat: "long", layout: "inline", className: "",
};
...
function formatRelativeTime(date: Date): string {
  ...
  if (diffDays >= 365) return `${Math.floor(diffDays / 365)} year${diffDays >= 730 ? "s" : ""} ago`;
  if (diffDays >= 30) return `${Math.floor(diffDays / 30)} month${diffDays >= 60 ? "s" : ""} ago`;
  if (diffDays >= 7) return `${Math.floor(diffDays / 7)} week${diffDays >= 14 ? "s" : ""} ago`;
...
function formatDate(isoDate: string, format: "short" | "long" | "relative"): string {
...
function usePostMeta(postId: string | undefined, isPreview: boolean): PostMeta | null {
  ...
  useEffect(() => {
    if (!isPreview || !postId) { setMeta(null); return; }
    let cancelled = false;
    fetch(`/api/posts/${postId}`)
      .then((res) => { if (!res.ok) throw new Error("fetch failed"); return res.json(); })
      .then((data) => {
        if (!cancelled) setMeta({
          publishedAt: data.publishedAt, categories: data.categories,
          tags: data.tags, wordCount: data.wordCount,
        });
      })
...
function buildClassName(content: PostInfoContent, layout: "inline" | "stacked"): string {
  return [
    "wp-block-post-info",
 ...
 */
function PostInfoRenderer({ content, styles, isPreview }: PostInfoRendererProps) {
  ...
  if ((content?.showDate ?? true) && meta.publishedAt) {
    items.push(
      <span key="date" className="flex items-center gap-1.5 text-sm text-gray-600">
  ...
  if ((content?.showCategories ?? true) && meta.categories?.length) {
    items.push(
      <span key="categories" className="flex items-center gap-1.5 text-sm text-gray-600">
        <Folder className="h-3.5 w-3.5" />
        <span className="flex flex-wrap gap-1">
          {meta.categories.map((cat) => <Badge key={cat}>{cat}</Badge>)}
        </span>
      </span>,
    );
  }
  if ((content?.showTags ?? true) && meta.tags?.length) {
    items.push(
      <span key="tags" className="flex items-center gap-1.5 text-sm text-gray-600">
  ...
  if (content?.showReadTime ?? true) {
    items.push(
      <span key="readtime" className="flex items-center gap-1.5 text-sm text-gray-600">
  ...
  if (items.length === 0) {
    return (
      <div className={className} style={styles}>
        <p className="text-sm text-gray-400 italic">No post info items enabled</p>
      </div>
    );
  }
...
export function PostInfoBlockComponent({ value, onChange, isPreview }: BlockComponentProps) {
  const { content, styles } = useBlockState<PostInfoContent>({
    value, getDefaultContent: () => DEFAULT_CONTENT, onChange,
  });
  return <PostInfoRenderer content={content} styles={styles} isPreview={isPreview} />;
}
...
function PostInfoSettings({ block, onUpdate }: PostInfoSettingsProps) {
  ...
  const content = accessor
    ? (accessor.getContent() as PostInfoContent)
    : (block.content as PostInfoContent) || DEFAULT_CONTENT;

  const updateContent = (updates: Partial<PostInfoContent>) => {
    if (accessor) {
      const current = accessor.getContent() as PostInfoContent;
      accessor.setContent({ ...current, ...updates });
      setUpdateTrigger((prev) => prev + 1);
    } else if (onUpdate) {
      onUpdate({
        content: { ...(block.content as Record<string, unknown>), ...updates } as unknown as BlockContent,
      });
    }
  };

  const toggles = [
    { id: "show-date", label: "Show Date", key: "showDate" as const, value: content?.showDate ?? true },
    { id: "show-categories", label: "Show Categories", key: "showCategories" as const, value: content?.showCategories ?? true },
    { id: "show-tags", label: "Show Tags", key: "showTags" as const, value: content?.showTags ?? true },
    { id: "show-readtime", label: "Show Read Time", key: "showReadTime" as const, value: content?.showReadTime ?? true },
  ];
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="space-y-4">
          {toggles.map((toggle) => (
            <div key={toggle.id} className="flex items-center justify-between">
              <Label htmlFor={toggle.id} className="text-sm font-medium text-gray-700">{toggle.label}</Label>
              <Switch id={toggle.id} checked={toggle.value} onCheckedChange={(checked) => updateContent({ [toggle.key]: checked })} />
            </div>
          ))}
        </div>
      </CollapsibleCard>
      ...
      <CollapsibleCard title="Format" icon={Settings} defaultOpen>
        <div className="space-y-4">
          <div>
            ...
            <Select value={content?.dateFormat ?? "long"} onValueChange={(val) => updateContent({ dateFormat: val as PostInfoContent["dateFormat"] })}>
          ...
          <div>
            ...
            <Select value={content?.layout ?? "inline"} onValueChange={(val) => updateContent({ layout: val as PostInfoContent["layout"] })}>
              <SelectTrigger className="mt-1 h-9 text-sm"><SelectValue /></SelectTrigger>
              <SelectContent>
      ...
      <CollapsibleCard title="Advanced" icon={Wrench} defaultOpen={false}>
        <div>
          <Label htmlFor="post-info-class" className="text-sm font-medium text-gray-700">Additional CSS Class(es)</Label>
          <Input id="post-info-class" value={content?.className || ""} onChange={(e) => updateContent({ className: e.target.value })} placeholder="e.g. custom-post-info" className="mt-1 h-9 text-sm" />
        </div>
      </CollapsibleCard>
    </div>
  );
...
function LegacyPostInfoRenderer({ block, isPreview }: { block: BlockConfig; isPreview: boolean }) {
  return (
    <PostInfoRenderer
