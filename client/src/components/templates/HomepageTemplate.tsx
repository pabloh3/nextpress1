...
export default function HomepageTemplate({ config, isPreview = false }: HomepageTemplateProps) {
  const { user } = useAuth();
  ...
  const mockStats = { posts: 5, pages: 3, comments: 12, users: 2 };
  const mockPosts = {
    posts: [
      { id: 1, title: "Welcome to NextPress", excerpt: "Getting started with your new WordPress-compatible CMS...", createdAt: new Date(), status: 'publish' },
      { id: 2, title: "Building Modern Websites", excerpt: "Learn how to create beautiful, responsive websites...", createdAt: new Date(), status: 'publish' },
      { id: 3, title: "Content Management Made Easy", excerpt: "Discover the powerful features of NextPress...", createdAt: new Date(), status: 'publish' }
    ]
  };

  const displayStats = isPreview ? mockStats : stats;
  const displayPosts = isPreview ? mockPosts : recentPosts;
  const displayUser = isPreview ? { firstName: "Demo", username: "user" } : user;
  ...
  const headerTitle = config.header?.title || `Welcome back, ${displayUser?.firstName || displayUser?.username || "User"}!`;
  const headerSubtitle = config.header?.subtitle || "Here's what's happening with your NextPress site today.";
  const backgroundColor = config.styling?.backgroundColor || "bg-wp-gray-light";
    ...
    <div className={`min-h-screen ${backgroundColor}`}>
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          {config.header?.showButtons !== false && (
              ...
              <CardContent>
                {displayPosts?.posts?.length > 0 ? (
                  <div className="space-y-4">
                    {displayPosts.posts.slice(0, 3).map((post: any) => (
